...
falls back to your **browser's local storage** — and the small pill next to
"Viewing as" in the top bar always tells you which mode you're in:

- **💾 Saved in this browser only** — what you'll always see in this
  exported package. Every status change, task assignment, note, and any
  event you add, edit or delete is saved automatically in the browser that
  made the change.
...
- Opening it in a **different browser or device** starts from the original
  ...
  not shared between people. This is expected, not a bug — the pill is
  there so it's never a mystery.

**If you need everyone on the team to see the same live, shared data**
(ticking a box on one laptop shows up on everyone else's, and the pill reads
**☁️ Synced — everyone sees the same data**), you have two options:
...
1. **Easiest: keep using the Claude-hosted version.** The version built for
   ...
   a fresh browser tab currently can't reach that shared store — the pill
   will read "Saved in this browser only" in that case too, which is a
...
2. **Add a small backend of your own.** `persist()`/`loadLocalState()` (task
   ...
   own infrastructure. `setSyncStatus()` (search for `syncStatus`) drives
   the pill and is a good template for wiring up your own backend's
   connected/error states the same way.

If the pill ever reads **⚠️ Save failed — kept locally only**, the last
change couldn't reach shared storage (e.g. a dropped connection) but is
safe in this browser and will retry on your next change.
...
("Viewing as" → Ray (Founder & Director) or COO in the top bar), use the
...
- **The roles.** Both the **"Viewing as"** switcher in the top bar and every
  **Team Member / Assign Task** field throughout the dashboard use the exact
  ...
  Creative Lead, Other. **Ray (Founder & Director)** and **COO** carry
  admin-level controls (marked 👑 in the switcher) — add/edit/delete events,
  assign or reassign tasks, and approve/send back/complete/reopen work; the
  other seven roles are team-member viewers who see only what's assigned to
  them. (To change who counts as an admin, edit the `ADMIN_ROLES` array next
  to `TEAM_MEMBERS` in `template.html` and rebuild.)
...
"My Tasks" page (see §6), full admin add/edit/delete for events and TCA
campaigns with a plain-language recurrence picker (see §3), and a per-event
detail panel with a status tracker, the same assignable task list, notes,
and a campaign-lead field. Every event auto-calculates its 14-day
