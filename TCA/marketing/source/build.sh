...
# with real favicons) from template.html + ../data/event_templates.json +
...
with open('../data/event_templates.json', 'r', encoding='utf-8') as f:
    events_json = f.read()
...
filled = template.replace("__EVENT_TEMPLATES_JSON__", events_json_safe).replace("__LOGO_SRC__", logo_src)
