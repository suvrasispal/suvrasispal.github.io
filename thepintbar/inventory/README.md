...
├── data/                  Inventory data
│   ├── inventory.seed.json  The supplied starting stock (82 products) — shipped
│   ├── inventory.xlsx     Created on first run from the seed; then the truth
...
## How the data works — read this first

**The Excel workbook is the single source of truth.** The browser does not keep
its own copy of the inventory. Every change is sent to the service, written to
`data/inventory.xlsx`, and the saved workbook is read straight back — what you
see on screen is always what is in the file.

- Open the app on a second browser, laptop or tablet and it loads the same
  current data. There is no per-browser inventory.
- The app re-reads the workbook every 8 seconds and whenever you switch back to
  the tab, so another person's changes appear without a manual refresh.
- Clearing browser cache or local storage loses nothing.
- If two devices save at the same moment, the second one is told the workbook
  moved on, its view refreshes to the newer data, and nothing is silently
  overwritten.
- If the service is unreachable, the app says so, refuses the change rather
  than pretending it saved, and keeps showing the last known good data until
  the service returns.

The status chip in the header shows which state you are in — green *Live*,
amber *Workbook unreachable*, or grey *This browser only*. Click it to refresh.

**Running `index.html` on its own (no service) means no sharing.** That mode is
fine for a demo on one machine, but for the bar you want Option A below.

---

## Option A — Run with the service (this is the one for the bar)

Full automatic Excel persistence, shared across every device. Every stock
movement writes straight into `data/inventory.xlsx`.
...
**First run** — the service builds `data/inventory.xlsx` automatically from the
shipped `data/inventory.seed.json` (82 products, 1,481 units, from the stock
list supplied 11 Sep 2026). The dashboard is populated the moment you open it.
From then on the workbook is the source of truth and the seed file is ignored.

**Using your own workbook instead** — drop it in as `data/inventory.xlsx`
before the first start, with sheets named `Inventory` and `Transactions`
(columns below). An existing workbook is never overwritten by the seed.
...
Double-click `index.html`. The dashboard loads the same 82-product starting
stock immediately — never blank. But **data lives in that browser only**, so
other devices will not see your changes. Use this for a demo or a quick look;
for service, run Option A.
...
POST /api/inventory   ←  { products, txns, rev }
                      →  { products, txns, rev, file }   ← the new truth
                      →  409 + the newer data, if rev was stale
```

`rev` is the workbook's modification time, used to detect a concurrent save.
The server de-duplicates on POST: same product id, or same name within the same
category, collapses to one row rather than creating a duplicate.

To move to a hosted database later, rewrite `readWorkbook()` and
`writeWorkbook()` and deploy the service somewhere the bar can reach. The front
end needs no change — same two endpoints, same payload.
...
- A save that does not reach the workbook is reported as failed, not applied.
- A malformed payload is rejected before it can overwrite a good workbook.
- Concurrent saves are serialised, and a stale save is refused with a 409.
